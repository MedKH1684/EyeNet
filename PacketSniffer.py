...
class PacketSniffer(QMainWindow):
	...
	def __init__(self):
		...
		#load template
		uic.loadUi("templates/APPTemplate.ui", self)
		...
		#firstWindow
		self.firstWindow = FirstWindow(self.currentMode)
	...
	def ipsWindowClicked(self):
		self.IPsWindow = IpsWindow(get_if_addr(conf.iface), self.ipsList)
	...
	def deleteFromDBClicked(self):
		if self.filterStatus:
			...
			self.ask5 = QMessageBox.question(self, "Delete From Database", "Do you want to delete all packets in the database ?")
			if self.ask5 == QMessageBox.Yes:
